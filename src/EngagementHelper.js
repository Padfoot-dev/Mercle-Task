...
const EngagementHelper = {
  engagementMessageOverTimeChartOptions: (messageCountList, channels) => {
    ...
    const filteredChannels = channels.filter(channel => groupedMessages[channel.id]?.length > 1);


    const series = filteredChannels.map(channel => {
      const channelId = channel.id;
      const data = groupedMessages[channelId].map(message => ({
    ...
    const options = {
      ...
      tooltip: {
        ...
        formatter: function () {
          return `<br><b>${this.series.name}</b></br>${this.y} Messages on ${Highcharts.dateFormat('%d %b', this.x)}`;
        },
      },
      series,
      plotOptions : {
